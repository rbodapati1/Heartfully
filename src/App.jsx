...
function SharePage({setPage,theme}){const[copied,setCopied]=useState(false);const appUrl="https://heartfully.app";async function copyLink(){try{await navigator.clipboard.writeText(appUrl);}catch{}setCopied(true);setTimeout(()=>setCopied(false),2500);}async function nativeShare(){if(navigator.share){try{await navigator.share({title:"Heartfully",text:"Heartfully — heartfelt messages, beautifully crafted.",url:appUrl});}catch{}}else{copyLink();}}const shareLinks=[{label:"WhatsApp",icon:"🟢",color:"#25d366",bg:"#edfaf2",url:`https://wa.me/?text=${encodeURIComponent("Check out this app: "+appUrl)}`},{label:"Facebook",icon:"📘",color:"#1877f2",bg:"#edf3fd",url:`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(appUrl)}`},{label:"X / Twitter",icon:"🐦",color:"#1da1f2",bg:"#e8f5fd",url:`https://twitter.com/intent/tweet?text=${encodeURIComponent("Heartfully — heartfelt daily messages — Heartfully 💛")}&url=${encodeURIComponent(appUrl)}`},{label:"LinkedIn",icon:"💼",color:"#0077b5",bg:"#e8f2fa",url:`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(appUrl)}`},{label:"Email",icon:"✉️",color:"#c9746b",bg:"#fdf0ee",url:`mailto:?subject=${encodeURIComponent("You should try Heartfully")}&body=${encodeURIComponent("I've been using this app to send meaningful daily messages.\n\n"+appUrl)}`},{label:"Instagram",icon:"📸",color:"#e1306c",bg:"#fdeef4",url:"#",note:"Copy link and share in your bio or story"}];const cards=[{icon:"🔍",title:"Google can find it",body:"Because this is a real website, Google indexes it. When someone searches 'daily heartfelt message app', Heartfully can appear organically — completely free.",tip:"SEO tip: Clear descriptions, regular updates, and genuine engagement all help."},{icon:"📱",title:"Add to Home Screen",body:"Anyone who visits on mobile can add it to their home screen — no app store, no download. It opens full-screen like a native app.",tip:"This is called a Progressive Web App (PWA). Starbucks and Pinterest work the same way."},{icon:"📣",title:"Word of mouth loop",body:"When someone receives a beautiful personalised note, they ask 'How did you do that?' — which leads directly to sharing the link.",tip:"The best growth is the message itself. Recipients become users."},{icon:"🔗",title:"A link is all it takes",body:"Unlike a native app, anyone can use Heartfully instantly by following a link — no download, no setup, no friction.",tip:"No friction = higher conversion. A link user sees value instantly."},{icon:"📰",title:"Social & content",body:"A simple Instagram or TikTok presence sharing example messages can drive thousands of visitors. Consistent, warm content builds a loyal audience.",tip:"Content ideas: daily message examples, relationship tips, 'how to tell someone you love them.'"},{icon:"📬",title:"QR codes",body:"A QR code pointing to the website URL can go on cards, printed materials, or presentations. Anyone who scans it lands directly in the app.",tip:"Generate a free QR code at qr.io or qrcode-monkey.com."}];return(<div style={{paddingTop:"60px",minHeight:"100vh",background:`linear-gradient(160deg,${theme.bg1},${theme.bg2})`}}><section style={{padding:"68px 22px 48px",textAlign:"center",borderBottom:`1px solid ${theme.border}`}}><h1 style={{fontFamily:"var(--fd)",fontSize:"clamp(28px,5vw,50px)",fontWeight:"700",color:"var(--text)",lineHeight:1.1,marginBottom:"12px"}}>Share the Love.<br/>No App Store Needed.</h1><p style={{color:"var(--sub)",fontSize:"15px",maxWidth:"500px",margin:"0 auto 28px",lineHeight:1.8}}>Being a web app isn't a limitation — it's a superpower. Here's how Heartfully gets discovered without any app store.</p><div style={{display:"flex",gap:"10px",justifyContent:"center",flexWrap:"wrap",marginBottom:"12px"}}><button className="btn" onClick={nativeShare} style={{background:"var(--accent)",color:"white",borderRadius:"100px",padding:"11px 24px",fontSize:"14px",boxShadow:`0 6px 20px ${theme.accent}44`}}>📤 Share this App</button><button className="btn" onClick={copyLink} style={{background:"var(--surface)",color:"var(--text)",border:`1.5px solid ${theme.border}`,borderRadius:"100px",padding:"11px 20px",fontSize:"14px"}}>{copied?"✓ Copied!":"🔗 Copy Link"}</button></div></section><div style={{maxWidth:"980px",margin:"0 auto",padding:"52px 22px 90px"}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:"9px",marginBottom:"60px"}}>{shareLinks.map((sl,i)=>(<a key={i} href={sl.url} target="_blank" rel="noopener noreferrer" className="share-card" style={{display:"flex",alignItems:"center",gap:"11px",padding:"13px 16px",background:sl.bg,border:`1.5px solid ${sl.color}22`,borderRadius:"14px",boxShadow:"0 2px 10px rgba(0,0,0,.05)",textDecoration:"none"}}><span style={{fontSize:"20px"}}>{sl.icon}</span><div><div style={{fontSize:"14px",fontWeight:"600",color:"var(--text)",fontFamily:"var(--fd)"}}>{sl.label}</div>{sl.note&&<div style={{fontSize:"11px",color:"var(--muted)",marginTop:"2px"}}>{sl.note}</div>}</div><span style={{marginLeft:"auto",color:sl.color,fontSize:"16px"}}>→</span></a>))}</div><h2 style={{fontFamily:"var(--fd)",fontSize:"clamp(22px,4vw,36px)",fontWeight:"600",color:"var(--text)",lineHeight:1.2,marginBottom:"28px",textAlign:"center"}}>Six ways Heartfully gets discovered without an app store</h2><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(276px,1fr))",gap:"16px",marginBottom:"52px"}}>{cards.map((c,i)=>(<div key={i} style={{background:"var(--surface)",borderRadius:"20px",padding:"24px",border:`1px solid ${theme.border}`,backdropFilter:"blur(8px)"}}><div style={{fontSize:"26px",marginBottom:"11px"}}>{c.icon}</div><h3 style={{fontFamily:"var(--fd)",fontSize:"16px",fontWeight:"600",color:"var(--text)",marginBottom:"9px"}}>{c.title}</h3><p style={{fontSize:"13px",color:"var(--sub)",lineHeight:1.8,marginBottom:"12px"}}>{c.body}</p><div style={{background:`${theme.accent}10`,border:`1px solid ${theme.accent}22`,borderRadius:"10px",padding:"9px 13px"}}><span style={{fontSize:"11px",color:"var(--accent)",fontWeight:"600"}}>💡 </span><span style={{fontSize:"12px",color:"var(--sub)",fontStyle:"italic"}}>{c.tip}</span></div></div>))}</div><div style={{background:"var(--dark)",borderRadius:"22px",padding:"36px"}}><h3 style={{fontFamily:"var(--fd)",fontSize:"20px",fontWeight:"600",color:"var(--darkText)",marginBottom:"22px"}}>📱 How to install Heartfully on your phone</h3><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:"14px"}}>{[{platform:"📱 iPhone / iPad",steps:["Open this website in Safari","Tap the Share button (box with arrow)","Scroll down → tap 'Add to Home Screen'","Give it a name → tap Add"]},{platform:"🤖 Android (Chrome)",steps:["Open this website in Chrome","Tap the three-dot menu (⋮) top right","Tap 'Add to Home Screen' or 'Install App'","Tap Add — done"]},{platform:"🖥️ Desktop (Chrome/Edge)",steps:["Open this website in Chrome or Edge","Look for the install icon (⊕) in the address bar","Click it and confirm installation","App appears in your taskbar / dock"]}].map((p,i)=>(<div key={i} style={{background:"rgba(255,255,255,0.06)",borderRadius:"15px",padding:"17px"}}><div style={{fontFamily:"var(--fd)",fontSize:"14px",fontWeight:"600",color:"var(--darkText)",marginBottom:"11px"}}>{p.platform}</div>{p.steps.map((s,j)=>(<div key={j} style={{display:"flex",gap:"9px",marginBottom:"7px",alignItems:"flex-start"}}><span style={{background:theme.accent,color:"white",borderRadius:"50%",width:"17px",height:"17px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px",fontWeight:"700",flexShrink:0,marginTop:"1px"}}>{j+1}</span><span style={{fontSize:"12px",color:`${theme.darkText}cc`,lineHeight:1.5}}>{s}</span></div>))}</div>))}</div></div></div><footer style={{background:"var(--dark)",padding:"24px 22px",textAlign:"center"}}><div style={{display:"flex",justifyContent:"center",gap:"16px",marginBottom:"7px",flexWrap:"wrap"}}><a href="mailto:hello@heartfully.app?subject=Feedback%20%2F%20Bug%20Report&body=What%20happened%3A%0A%0AWhat%20I%20expected%3A%0A%0ADevice%20%2F%20browser%3A" style={{color:"rgba(255,255,255,0.35)",fontSize:"12px",textDecoration:"none"}} onMouseEnter={e=>e.target.style.color="rgba(255,255,255,0.7)"} onMouseLeave={e=>e.target.style.color="rgba(255,255,255,0.35)"}>💬 Send feedback</a></div><p style={{color:`${theme.darkText}40`,fontSize:"12px"}}>© Heartfully · All personal data stored locally on your device</p></footer></div>);}
